...
type test struct {
	workdir        string
	args           []string // additional arguments to pass to command.
	goldenFilePath string
	wantExitCode   int
}

func runE2ETest(t *testing.T, tests []test, testFn func(t *testing.T, tt test, originalWorkDir string, goLicensesPath string)) {
	...
	for _, tt := range tests {
		t.Run(tt.workdir, func(t *testing.T) {
			...
			testFn(t, tt, originalWorkDir, goLicensesPath)
		})
	}
}

func runTestReportCommand(t *testing.T, tt test, originalWorkDir string, goLicensesPath string) {
...
func TestReportCommandE2E(t *testing.T) {
	tests := []test{
		{"testdata/modules/hello01", nil, "licenses.csv", 0},
		{"testdata/modules/cli02", nil, "licenses.csv", 0},
		{"testdata/modules/vendored03", nil, "licenses.csv", 0},
		{"testdata/modules/replace04", nil, "licenses.csv", 0},

		{"testdata/modules/hello01", []string{"--template", "licenses.tpl"}, "licenses.md", 0},
		{"testdata/modules/template01", []string{"--template", "licenses.tpl"}, "licenses.md", 0},
	}

	runE2ETest(t, tests, runTestReportCommand)
}

func runTestCheckCommand(t *testing.T, tt test, originalWorkDir string, goLicensesPath string) {
...
func TestCheckCommandE2E(t *testing.T) {
	...
	runE2ETest(t, tests, runTestCheckCommand)
}
